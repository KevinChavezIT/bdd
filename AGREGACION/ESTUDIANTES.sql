...
select col.id_colegio as "codigo_colegio",col.nombre,count (*) 
as "total_estudiantes" from estudiantes est, colegio col
where col.id_colegio = est.id_colegio 
group by col.id_colegio

select round((extract(year from current_date) - extract(year from fecha_nacimiento)))
